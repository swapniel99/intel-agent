...
TOOL_NAMES = {"fetch_tech_news", "manage_local_library", "render_prefab_dashboard"}
...
async def test_render_basic_dashboard():
    data = _parse(await _call("render_prefab_dashboard", {
...
async def test_render_with_radar_chart():
    data = _parse(await _call("render_prefab_dashboard", {
        ...
        "chart": {
            ...
            "series": [{"data_key": "val", "label": "Score"}],
        },
        "ai_answer": "Radar.",
    }))
    assert data.get("status") == "dashboard_ready"
...
async def test_render_empty_cards():
    data = _parse(await _call("render_prefab_dashboard", {
        "cards": [], "topic": "empty", "ai_answer": "No articles."
    }))
    assert data.get("status") == "dashboard_ready"


@pytest.mark.asyncio
async def test_render_theme_keys():
    for theme in ["ai", "security", "rust", "python", "linux", "default"]:
        data = _parse(await _call("render_prefab_dashboard", {
            "cards": [], "topic": theme, "theme_key": theme, "ai_answer": f"Theme {theme}."
        }))
        assert data.get("status") == "dashboard_ready", f"Failed theme: {theme}"
